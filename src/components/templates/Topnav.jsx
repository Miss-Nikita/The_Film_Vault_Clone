      ...
      <div className="  h-[10vh] relative  flex justify-start items-center ml-[15%]  ">
        ...
        {query.length > 0 && (
         ...
         <div className="absolute w-[50%] max-h-[50vh] bg-zinc-200  top-[90%] overflow-auto rounded ">
          {searches.map((s, i) =>(
            <Link key={i} className="hover:text-black hover:bg-zinc-400 duration-300 font-semibold text-zinc-500  w-[100%] p-5 flex justify-start items-center border-b-2 border-zinc-100">
            <img src="" alt="" />
            <span>{s.name || s.title || s.orignal_name || s.orignal_title} </span>
          </Link>
          ))}
